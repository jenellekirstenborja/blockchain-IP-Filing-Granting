<template>
  <div id="app">
    <b-navbar toggleable="lg" type="" variant="" class="navbar-custom">
      <b-container fluid>
        <b-row>
          <b-col cols="auto">  <b-navbar-brand href="#">
              <img src="../assets/logo.png" alt="Logo" class="logo-img"> 
              <!-- <p class="header">Blockchain-based Intellectual Property Filing and Granting System</p> -->
            </b-navbar-brand>
          </b-col>

          <b-col class="ml-auto">
            <div class="user-text-container ml-auto">
              <b-navbar-nav>
                <b-navbar-nav>
	...
	<b-button class="ml-auto mr-auto" v-b-modal.modal-prevent-closing v-if="role === 2">Apply</b-button>
		...
		<b-tabs content-class="mt-3">
			...
			<b-tab title="View All" active>
    <!-- Search input field -->
    
    <!-- Table to display all patent applications -->
    <b-table striped hover :items="filteredApplications" :fields="fields">
        ...
        <template v-slot:cell(action)="data">
			...
			<b-tab title="Pending" active>
				...
				<b-table striped hover :items="pendingList" :fields="fields">
					...
					<template v-slot:cell(action)="data">
						...
						<b-button v-if="role === 1" class="mr-2" variant="danger" @click="denyPatentApp(data.item.applicationNumber)">Deny</b-button>
			...
			<b-tab title="Granted">
				...
				<b-table striped hover :items="grantedList" :fields="fields">
		...
		<b-tab title="Denied">
				<!-- Table to display granted patents -->
				<b-table striped hover :items="deniedList" :fields="fields">
					<!-- Action buttons for granted patents -->
					<template v-slot:cell(action)="data">
						<b-link href="#" v-b-modal.patent-details @click="handleOpenPatentDetailsModal(data.item)">Details</b-link>
					</template>
...
export default {
    ...
    filteredApplications() 	{
			if (this.searchQuery) {
				...
				return this.allList.filter((patent) => {
					return (
    ...
    for (let i = 0; i < this.deniedPatentsLength; i++) {
        ...
        var _deny = {
            ...
            applicationDate: moment(deny.applicationDate * 1000).format("MMMM Do YYYY, h:mm:ss a"),
            publicationDate: moment(deny.publicationDate * 1000).format("MMMM Do YYYY, h:mm:ss a"),
...
async getAllList() {
    ...
    try {
        ...
        // Remove the denied patent from the pending list
        const index = this.pendingList.findIndex((patent) => patent.applicationNumber === applicationNumber);
        if (index !== -1) {
            this.pendingList.splice(index, 1);
        }
        ...
        console.error("Error denying patent application:", error);
    }
},
...
.user-text-container {  
  ...
  height: auto;
  margin-right: 20px;
  background-color: pink/* Use ml-auto class for right positioning */
}
.admin-text {
	font-size: 1.1em;
	letter-spacing: 1.2px;
